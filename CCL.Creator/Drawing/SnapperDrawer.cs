...
namespace CCL.Creator.Drawing
{
    internal class SnapperDrawer
    {
        ...
        private static void DrawGizmoParticleSystem(LineHandSnapperProxy snapper, GizmoType gizmoType)
        {
            ...
            var from = Quaternion.Euler(0, snapper.minAngle, 0) * Vector3.left;
            var angle = snapper.maxAngle - snapper.minAngle;

            Handles.matrix = snapper.lineStart.localToWorldMatrix;
            Handles.color = Color.cyan;

            Handles.DrawLine(Vector3.zero, end);
            Handles.DrawSolidArc(Vector3.zero, Vector3.up, from, angle, 0.01f);
            Handles.DrawSolidArc(end, Vector3.up, from, angle, 0.01f);

            angle = 360 - angle;
            Handles.DrawWireArc(Vector3.zero, Vector3.up, from, -angle, 0.01f);
            Handles.DrawWireArc(end, Vector3.up, from, -angle, 0.01f);
